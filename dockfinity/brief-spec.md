...
AT MOST SIX ITEMS across all lanes. In practice a good day is two or three.
Six is a ceiling, not a target, and a brief padded to reach it is worse than a
short one.
...
6.1 TOP-LEVEL FIELDS
  ...
  thesis        the connecting thread, one sentence
  accentPhrase  a phrase that ALREADY EXISTS inside thesis, word for word.
                ...
                order to be accented, and the validator checks that thesis
                contains it exactly.
